...
class Replay:
    def __init__(self, path='Replay/', save=True, batch_size=1, device='cpu', num_workers=0, offline=True, stream=False,
        ...
        # TODO Worker needs to terminate, parent too when any terminate
...
class PrefetchTape:
    def __init__(self, batch_size, device, gpu_prefetch_factor=0, prefetch_factor=3, pin_memory=False):
        ...
        self.one_time = torch.tensor(False, dtype=torch.bool).share_memory_()
    ...
    def get_batch(self):
        ...
        clock = time.time()
        while True:
            if self.start_index < next_start_index <= self.end_index or \
                    self.end_index <= self.start_index < next_start_index:
                experiences = self.prefetch_tape[self.start_index:next_start_index]
                break
            elif next_start_index <= self.end_index <= self.start_index:
                if next_start_index == self.end_index == self.start_index and not self.one_time:
                    self.one_time[...] = True  # When cap == batch size, avoid infinite looping on the same batch
                experiences = self.prefetch_tape[self.start_index:] + self.prefetch_tape[:next_start_index]
                break
            elif time.time() - clock > 30:
                # print(self.start_index, next_start_index, self.end_index)
        ...
        # Collate
        batch = {key: torch.stack([torch.as_tensor(experience[0][key][...]).to(self.device, non_blocking=True)
                                   for experience in experiences]) for key in experiences[0][0]}
