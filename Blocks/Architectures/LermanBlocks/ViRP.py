...
class ViRP(ViT):
    def __init__(self, input_shape, patch_size=4, out_channels=32, heads=8, depth=3, pool='cls', output_dim=None,
        ...
        elif experiment == 'plus_in_concat_plus_mid':  # see if attention is useful for "reasoning"
            core = RelationSimplestV3
        elif experiment == 'head_wise_ln':  # disentangled relational reasoning
            core = RelationDisentangled
        elif experiment == 'head_in_RN':  # invariant relational reasoning between input-head
            core = RelationSimpler
        elif experiment == 'head_head_in_RN':  # relational reasoning between heads too
