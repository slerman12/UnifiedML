...
from Blocks.SelfSupervisors import DynamicsSelfSupervisor, SelfSupervisedIntercept
...
class AC2Agent(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Dynamics self-supervised learning
        if self.depth and not self.generate:
            self.encoder.Eyes = SelfSupervisedIntercept(self.encoder.Eyes)
            self._supervisor = DynamicsSelfSupervisor()
    ...
    def learn(self, replay):
        ...
        if self.RL:
            ...
            if self.generate:
                ...
                if self.log:
                    logs['discriminator_real'] = critic_loss
            ...
            # Dynamics loss
            dynamics_loss = self._supervisor

            # "Sharpen Foresight"

            # Update critic, dynamics
            Utils.optimize(critic_loss + dynamics_loss, self.critic, retain_graph=self.generate,
                           epoch=self.epoch if replay.offline else self.episode)

        # Update encoder
        Utils.optimize(None,  # Using gradients from previous losses
                       self.encoder, self._supervisor, epoch=self.epoch if replay.offline else self.episode)
