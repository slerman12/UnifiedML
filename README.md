...
[comment]: <> (*Note:* ```RL=false``` sets training to **Standard** supervised-only classification. Without ```RL=false```, an **Augmented RL** update joins the supervised learning update s.t. ```reward = -error```.)
