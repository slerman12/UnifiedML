...
```Run.py``` handles training and evaluation loops, saving, logging, plotting.
...
The above returns a 93% on CIFAR-10 with a ResNet18 backbone, which is pretty good. Changing datasets/architectures is as easy as modifying the corresponding parts ```task=``` and ```Eyes=``` of the above script.
...
*Visual* means 2d spatial, *auditory* means 1d spatio-temporal, *proprioceptive* means 1d permutable, and *temporal* means an (N+1)d sequentiality.

By noun, they are referred to as *Vision*, *Audio*, *Proprioception*, and *Pulse* respectively.

Different tasks support different sensory types. For example, RL typically supports visual (image-based) or proprioceptive (state-based), temporal (like video) or non-temporal (frame-stacked). If a flag is passed in that is not supported by the task, e.g. ```auditory=true```, a runtime error is triggered. If no flags are passed, then the task’s defaults are used: for example, visual, non-temporal. In other words, you can say ```proprioceptive=true``` to get state embeddings in RL, or leave it be to get the usual frame-stacked images.

```
python Run.py proprioceptive=true
```

An audio classification example:
```
python Run.py task=classify/audio/bla
```
...
The easiest way to do distributed training is to use the ```parallel=true``` flag,
...
[comment]: <> (python Run.py learn_per_steps=0 replay.save=true load_per_steps=1 )

[comment]: <> (```)

[comment]: <> (```console)

[comment]: <> (python Run.py offline=true save_per_steps=2)
...
[comment]: <> (The order in which these are run matters, lest the replays be saved to and loaded from different paths, potentially.)
